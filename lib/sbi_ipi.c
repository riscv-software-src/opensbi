			...
			u32 event, void *data)
{
	...
	if (event == SBI_IPI_EVENT_SFENCE_VMA ||
		...
		ipi_tlb_fifo = sbi_tlb_info_fifo_ptr(remote_scratch);
		ret = sbi_fifo_enqueue(ipi_tlb_fifo, data);
		while (ret < 0) {
			/**
...
void sbi_ipi_process(struct sbi_scratch *scratch)
{
	...
	const struct sbi_platform *plat = sbi_platform_ptr(scratch);
	struct sbi_fifo *ipi_tlb_fifo = sbi_tlb_info_fifo_ptr(scratch);
	...
	do {
		...
		case SBI_IPI_EVENT_SFENCE_VMA_ASID:
			tinfo = sbi_fifo_dequeue(ipi_tlb_fifo);
			while (tinfo) {
...
{
	struct sbi_fifo *tlb_info_q = sbi_tlb_info_fifo_ptr(scratch);

	sbi_ipi_data_ptr(scratch)->ipi_type = 0x00;
	sbi_fifo_init(tlb_info_q, SBI_TLB_INFO_MAX_QUEUE_SIZE);
